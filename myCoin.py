...
# myCoin-Bitcoin-Wallet



try:
    import binascii, ecdsa, time, hashlib, base58, blocksmith 
except ImportError:
    import subprocess
...
while loop:
    ...
    if menu == 1:
        ...
        phrase = input("Enter your uniqe 12/24/25/26 phrases here: ")
        mail = input("Enter your e-mail address: ")
        print("|*******************************************************Bitcoin-Wallet**************************************************************************|"+"\n"+"\n")
        secret = (str(phrase) + ' ' + str(mail))
        pvt = hashlib.sha256(secret.encode('utf-8')).hexdigest()
        pvt1 = hashlib.sha256(pvt.encode('utf-8')).hexdigest()
        pvt2 = hashlib.sha256(pvt1.encode('utf-8')).hexdigest()
        privatekey = hashlib.sha256(pvt2.encode('utf-8')).hexdigest()
    ...
    if menu == 2:
        print("Menu 2 has been selected - Etherum Wallet")
        phrase = input("Enter your uniqe 12/24/25/26 phrases here: ")
        mail = input("Enter your e-mail address: ")
        print("|*******************************************************Etherum-Wallet**************************************************************************|"+"\n"+"\n")
        secret = (str(phrase) + ' ' + str(mail))
        pvt = hashlib.sha256(secret.encode('utf-8')).hexdigest()
        pvt1 = hashlib.sha256(pvt.encode('utf-8')).hexdigest()
        pvt2 = hashlib.sha256(pvt1.encode('utf-8')).hexdigest()
        ...
        file =  open("Etherum Wallet.txt","a")
        file.write("We only have what we give." + "- Isabel Allende" + "\n" + "Donate: BTC - 13vdSzghad1KWymvMDDPixN3ktdkZWaS7f" + "\n"+ "ETH - 0x2b6F5a72f7825bC5a2698681CE7a8c0da49AF75c"+"\n"+"\n"+"\n")
        file.write("Phrases: " + str(phrase) + "\n" + "E-mail: " + str(mail) + "\n" + "Privatekey: " + " " + str(privatekey)+ "\n" + "Address:    " + " " + str(address)+ "\n" + "Checksum_a: "+ " " +str(checksum_address)+"\n"+"\n")
        file.close()
